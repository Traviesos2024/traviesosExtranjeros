...
//! Falta importar aqui los modelos correctos (chat y user)
...
const toggleLikeExperience = async (req, res, next) => {
  try {
    ...
    if (req.user.experienceFav.includes(idExperience)) {
      try {
        await User.findByIdAndUpdate(_id, {
          //Actualiza el usuario e incluye su experiencia fav
          $pull: { experienceFav: idExperience },
        });
        ...
        return res.status(404).json({
          error: "No se ha actualizado el usuario -  experienceFav",
          message: error.message,
        });
      }
    } else {
      try {
        await User.findByIdAndUpdate(_id, {
          ...
          $push: { experienceFav: idExperience },
        });
        ...
        try {
          ...
          return res.status(200).json({
            action: "like",
            user: await User.findById(_id).populate("experienceFav"),
            movie: await Experience.findById(idExperience).populate("likes"),
          });
        } catch (error) {
          return res.status(404).json({
